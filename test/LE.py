...
for j in range(4,90,5):
    LEoutput = manifold.SpectralEmbedding(n_components=compo, n_neighbors=j).fit_transform(co)
    print("standrdok")
    LEoutput = LEoutput[0:100]
    ...
    for i in range(490):
        ...
        LEoutput = manifold.SpectralEmbedding(n_components=compo, n_neighbors=j).fit_transform(coordinate)
        LEoutput = LEoutput[0:100]
